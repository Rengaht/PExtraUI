...
public:
	ScenePreview(ofApp *set_ptr,GlobalSource *source_):SceneBase(set_ptr,source_,"preview.png",301){

		_img_redo.loadImage("preview_2.png");
	...
	void drawFront(){
		if(_can_redo){
			_img_redo.draw(0,0,ofGetWidth(),ofGetHeight());
		}
	}
	void init(){
		SceneBase::init();
	...
	void update(float dt_){
		...
		if(_status==SceneStatus::Due) _arr_button[0].setEnable(_can_redo);
